...
.Procedure
. From the left navigation menu, select *Platform Configuration -> System Policies*.
. Scroll down to `Fixable CVSS >= 7` (`rhacs_fix_image_vuln_policy`):
+
image::images/rhacs_fix_image_vuln_policy.png[RHACS Policy - Configure]
+
...
. Review the other attributes, and note the `Rationale` and `Remediation` fields, which you can expect to see in the policy output later.
. In the upper right, click *Edit*.
. Click the *Enabled* switch to make sure that the policy is `Enabled`.
. Click *Next* to review the policy criteria.
. Click *Next* to see a _preview_ of policy violations, then *Next* again to review the enforcement options.
. Click the *ON* switch for *BUILD* time enforcement and then *Save*.
