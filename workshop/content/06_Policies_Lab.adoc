...
+
image::rhacs_policy_overview.png[RHACS Policy Overview]

. Select the `Alpine Linux Package Manager (apk) in Image` policy.
. On the right, examine the policy details:
+
image::rhacs_policy_specific_policy.png[RHACS Policy Specific]
...
+
image::rhacs_policy_category_graph.png[RHACS Policy Category Graph]
+
...
+
image::rhacs_policy_category_graph_list.png[RHACS Policy Category Graph List]
+
...
+
image::rhacs_policy_categories_filter_investigation.png[RHACS Policy Filter includes Category Investigation]
